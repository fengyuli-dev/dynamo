...
/// This frontend uses the single combined audit record model: one record per
/// chat completion, finalized at response time and carrying both the request and
/// response payloads (plus captured HTTP headers). It is emitted as a single
/// OTLP log record labeled `response` for downstream parity with the two-record
/// model's response event.
const AUDIT_EVENT_BODY: &str = "openai.response";
const AUDIT_EVENT_TYPE: &str = "response";
...
impl AuditSink for OtelSink {
    ...
    async fn emit(&self, rec: &AuditRecord) {
        ...
        record.set_body(AnyValue::String(AUDIT_EVENT_BODY.into()));
        record.add_attribute("rid", AnyValue::String(rec.request_id.clone().into()));
        record.add_attribute("event_type", AnyValue::String(AUDIT_EVENT_TYPE.into()));
